...
Epics 01–02 implemented (see `docs/plan/` for the implementation plans and per-epic manual testing steps):
...
Setup: `npm install && npm run dev`. To wire the backend: run `supabase/migrations/00000000000001_initial_schema.sql` in the Supabase SQL Editor, deploy the three Edge Functions, and copy `.env.example` to `.env.local`.
